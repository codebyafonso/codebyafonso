```bash
██████╗ ██╗   ██╗     █████╗ ███████╗ ██████╗ ███╗   ██╗███████╗ ██████╗ 
██╔══██╗╚██╗ ██╔╝    ██╔══██╗██╔════╝██╔═══██╗████╗  ██║██╔════╝██╔═══██╗
██████╔╝ ╚████╔╝     ███████║█████╗  ██║   ██║██╔██╗ ██║███████╗██║   ██║
██╔══██╗  ╚██╔╝      ██╔══██║██╔══╝  ██║   ██║██║╚██╗██║╚════██║██║   ██║
██████╔╝   ██║       ██║  ██║██║     ╚██████╔╝██║ ╚████║███████║╚██████╔╝
╚═════╝    ╚═╝       ╚═╝  ╚═╝╚═╝      ╚═════╝ ╚═╝  ╚═══╝╚══════╝ ╚═════╝ 
```

```bash
[codebyafonso@github ~]$ whoami
```

> Desenvolvedor focado em construir ferramentas que resolvem problemas reais.  
> Back-end por natureza. Automação por obsessão. Full-stack quando necessário.

```bash
[codebyafonso@github ~]$ cat stack.txt
```

```
Languages   →  TypeScript · Python · JavaScript
Runtime     →  Node.js · Express · NestJS
Database    →  MongoDB · PostgreSQL
Frontend    →  React · Tailwind CSS
Infra       →  Docker · Linux · Nginx
Tools       →  Git · REST APIs · WebSockets
```

```bash
[codebyafonso@github ~]$ ls projects/
```

```
motosync/          → Rastreador de manutenção de motos   [TS · Node · MongoDB]
pppoeye/           → Dashboard de monitoramento PPPoE    [Python · MikroTik]
oltEye/            → Monitor de OLTs/ONUs via Telnet     [Python · Telnet]
supportflow/       → Sistema de suporte N1               [JS · Node]
surfpe/            → Plataforma de gestão               [TS · React]
```

```bash
[codebyafonso@github ~]$ cat focus.txt
```

```
→  automação de processos
→  monitoramento de redes (MikroTik · OLT · PPPoE)
→  APIs REST robustas
→  bots e integrações (Telegram · WhatsApp)
→  análise de dados com Python
```

```bash
[codebyafonso@github ~]$ git log --oneline --all
```

<div align="center">

<img height="160em" src="https://github-readme-stats.vercel.app/api?username=codebyafonso&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&include_all_commits=true"/>
<img height="160em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=codebyafonso&layout=compact&theme=tokyonight&hide_border=true&langs_count=6"/>

</div>

```bash
[codebyafonso@github ~]$ echo $CONTACT
```

```
github   →  github.com/codebyafonso
email    →  subscriptions@tds.company
```

```bash
[codebyafonso@github ~]$ █
```
